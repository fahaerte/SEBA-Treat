...
export const SentMealReservation = ({mealOfferId, sellerId, reservation}: SentMealOfferRequestBottomProps) => {
    ...
    async function updateReservationState(newState: MealReservationState) {
        try {
            await MealOfferService.updateMealReservationState(String(mealOfferId), reservation._id, newState);
    ...
    function getActionButton() {
        ...
        if (reservationState == MealReservationState.PENDING) {
            ...
            label = "Buyer confirmed TODO this"
        }
        return <Button onClick={() => updateReservationState(newState)} disabled={disabled}>{label}</Button>;
    }

    function getActionBar() {
        if (reservationState == MealReservationState.BUYER_CONFIRMED ||
            ...
            return (
                <Row>
                    <Col className={"col-sm-auto"}>
                        <Button onClick={() => updateReservationState(MealReservationState.BUYER_REJECTED)}>Cancel
                            Request</Button>
                    </Col>
                    <Col className={"col-sm-auto"}>
                        {getActionButton()}
