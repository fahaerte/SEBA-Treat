...
export const SentMealReservation = ({
    ...
    function getActionElement() {
        if (reservationState == MealReservationState.PENDING) {
            return (
                <span>Wait for the seller to respond to your request</span>
            );
        } else if (reservationState == MealReservationState.SELLER_ACCEPTED) {
            return (
                <Button onClick={() => updateReservationState(MealReservationState.BUYER_CONFIRMED)}>Confirm
                    Request</Button>
            )
        } else if (reservationState == MealReservationState.SELLER_REJECTED) {
            return (
                <span>The seller rejected your request</span>
            );
        } else if (reservationState == MealReservationState.BUYER_REJECTED) {
            return (
                <span>You cancelled your request</span>
            );
            ...
            return (
                <Row>
                    <Col className={"col-sm-auto"}>
                        <Button color={"light"}
                                onClick={() => updateReservationState(MealReservationState.BUYER_REJECTED)}>Cancel Request
                        </Button>
                    </Col>
                    <Col className={"col-sm-auto align-items-center d-flex"}>{getActionElement()}</Col>
                </Row>
            );
        }
    }
