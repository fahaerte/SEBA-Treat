...
const create = Joi.object({
    ...
    categories: Joi.array().min(1).items(Joi.string().valid(...Object.values(MealCategory))).required(),
    allergens: Joi.array().items(Joi.string().valid(...Object.values(MealAllergen))),
...
const updateReservationState = Joi.object({
        reservationState: Joi.string().valid(...Object.values(MealReservationStateEnum)).required()
    }
)
