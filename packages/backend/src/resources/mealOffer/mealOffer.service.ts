...
class MealOfferService {
    ...
    public async updateMealOfferReservationState(
        ...
        if (newState === MealReservationState.SELLER_ACCEPTED) {
            await this.updateMealOfferReservationToSellerAccepted(mealOfferId, user, mealReservationId);
        } else if (newState === MealReservationState.SELLER_REJECTED) {
            await this.updateMealOfferReservationToSellerRejected(mealOfferId, user, mealReservationId);
        } else if (newState === MealReservationState.BUYER_CONFIRMED) {
            await this.updateMealOfferReservationToBuyerConfirmed(mealOfferId, user, mealReservationId);
        } else if (newState === MealReservationState.BUYER_REJECTED) {
            await this.updateMealOfferReservationToBuyerRejected(mealOfferId, user, mealReservationId);
        } else if (newState === MealReservationState.PENDING) {
            throw new InvalidMealReservationStateException(`State can not be set to ${MealReservationState.PENDING}`)
        } else {
    ...
    private async updateMealOfferReservationToSellerAccepted(
        ...
        const [mealOffer, mealReservation] =
            await this.getMealOfferAndReservationForSeller(mealOfferId, mealReservationId, seller) as [MealOffer, MealReservation];
        if (mealReservation.reservationState === MealReservationState.PENDING) {
            ...
            throw new InvalidMealReservationStateException(`State should be ${MealReservationState.PENDING}`);
        }
    }
    ...
    private async updateMealOfferReservationToBuyerConfirmed(
        ...
        const [mealOffer, mealReservation] =
            await this.getMealOfferAndReservationForBuyer(mealOfferId, mealReservationId, buyer) as [MealOffer, MealReservation];
        console.log(mealReservation);
        if (mealReservation.reservationState === MealReservationState.SELLER_ACCEPTED) {
            ...
            await this.mealTransactionService.performTransaction(mealTransaction._id as ObjectId);
            mealReservation.reservationState = MealReservationState.BUYER_CONFIRMED;
            await mealOffer.save();
        } else {
            throw new InvalidMealReservationStateException(`State should be ${MealReservationState.SELLER_ACCEPTED}`);
        }
    }
    ...
    private async updateMealOfferReservationToSellerRejected(
        ...
        if (mealReservation.reservationState === MealReservationState.PENDING ||
            ...
            throw new InvalidMealReservationStateException(`State should be ${MealReservationState.PENDING} or ${MealReservationState.SELLER_ACCEPTED}`);
        }
    }
    ...
    private async getMealOfferAndReservationForSeller(
        ...
        const [mealOffer, mealReservation] =
            await this.getMealOfferAndReservation(mealOfferId, mealReservationId) as [MealOffer, MealReservation];
        if (String(mealOffer.user) === String(seller._id)) {
            return [mealOffer, mealReservation];
        }
