...
class MealOfferService {
    ...
    public async createMealOfferReservation(
        ...
        if (!user._id.equals(mealOffer.user)) {
            ...
            const result = reservations.find((reservation) => user._id.equals(reservation.buyer))
            if (result === undefined) {
                mealOffer.reservations.push({buyer: user._id} as MealReservation);
                return await mealOffer.save();
            }
            throw new InvalidMealReservationException("You can only make one reservation per meal offer");
        }
        throw new InvalidMealReservationException("You can not make a reservation for your own meal offer");
    }
